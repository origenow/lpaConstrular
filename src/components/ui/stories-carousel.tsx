...
export const Story = ({ className, ...props }: StoryProps) => (
  <CarouselItem className={cn('basis-auto !w-[200px] pl-2 md:pl-4', className)}>
    <div
      className={cn(
        'group relative overflow-hidden rounded-xl bg-slate-100/40',
...
export const StoryVideo = ({ className, ...props }: StoryVideoProps) => {
  ...
  return (
    <video
      className={cn('absolute inset-0 size-full object-cover transition-opacity duration-200 group-hover:opacity-90', className)}
      loop muted preload="metadata" ref={videoRef} tabIndex={0}
      onMouseOver={() => videoRef.current?.play()}
      onMouseOut={() => { if (videoRef.current) { videoRef.current.pause(); videoRef.current.currentTime = initialTimeRef.current; } }}
      onFocus={() => videoRef.current?.play()}
      onBlur={() => { if (videoRef.current) { videoRef.current.pause(); videoRef.current.currentTime = initialTimeRef.current; } }}
      {...props}
    />
  );
};
